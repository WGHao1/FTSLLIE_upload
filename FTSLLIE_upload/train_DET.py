...
class ModelNSNet(nn.Module):
    def __init__(self, model1, model2, model3):
        super().__init__()
        self.texture_loss = models_vevid.SSIML1Loss(channels=1)
        ...
        self.model_fusenet = model3(in_channels=5, out_channels=1)
    ...
    def forward(self, image, image_gt, training=True):
        ...
        with torch.no_grad():
            ...
            for strength in [0, illumi_in, 2*illumi_in, 3*illumi_in, 4*illumi_in]:
        ...
        restor_loss = self.l2_loss(texture_fuse, texture_gt)
        ssim_loss = 0.4875 * self.ssim_loss(texture_fuse, texture_gt)

        return noisy_gt, texture_nss, texture_fuse, texture_res, illumi, restor_loss, ssim_loss
...
def train(opt):
    ...
    if opt.optim == 'adam':
        optimizer = torch.optim.Adam(model.module.model_fusenet.parameters(), opt.lr)
    else:
        optimizer = torch.optim.SGD(model.module.model_fusenet.parameters(), opt.lr, momentum=0.9, nesterov=True)

    scheduler = CosineLR(optimizer, opt.lr, opt.num_epochs)
    epoch = 0
    step = 0
    model.module.model_fusenet.train()
    ...
    try:
        for epoch in range(opt.num_epochs):
            ...
            if not opt.sampling:
                for iter, (data, target, name) in enumerate(progress_bar):
                    ...
                    try:
                        ...
                        illumi, restor_loss, ssim_loss = model(data, target, training=True)

                        loss = restor_loss +ssim_loss
            ...
            if epoch % opt.val_interval == 0:
                model.module.model_fusenet.eval()
                loss_ls = []
                psnrs = []
                ssims = []
                ...
                for iter, (data, target, name) in enumerate(val_generator):
                    with torch.no_grad():
                        ...
                        noisy_gt, texture_nss, texture_fuse, texture_res, \
                        illumi, restor_loss, ssim_loss = model.module(data, target, training=False)
                ...
                save_checkpoint(model,
                                f'{"loss"}_{loss}_{"NFM"}_{"%03d" % epoch}_{"psnr"}_{psnr}_{"ssim"}_{ssim}.pth')
...
def save_checkpoint(model, name):
    if isinstance(model, nn.DataParallel):
        ...
        torch.save(model.model_fusenet.state_dict(), os.path.join(opt.saved_path, name))
